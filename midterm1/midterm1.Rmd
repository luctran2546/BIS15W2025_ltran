...
Problem 4. (2 points) Which habitat has the greatest species diversity? (meaning number of species)
```{r}
ecosphere %>% 
  group_by(habitat) %>% 
...
ecosphere %>% 
  ...
  summarize(
    min_winter_range_area = min(winter_range_area),
...
ecosphere %>% 
  ...
  arrange(desc(average_mass_g)) #for legibility/ease of reading
```
...
ecosphere %>% 
  ...
  summarize(
    average_population_size = mean(population_size, na.rm=T),
    .groups = 'keep'
  )
```
...
ecosphere %>% 
  ...
  filter(diet == "Vertebrates", 
         ...
         urban_affiliate == "No", 
         migratory_strategy == "Long") %>% 
  filter(mass_g == max(mass_g)) #selecting for the one at highest risk (highest mass)
```
...
ecosphere %>% 
  mutate(
    conservation_status = ifelse(population_size > 300000, "stable", "threatened")) %>% 
  arrange(desc(population_size))
#Problem: about half of the species listed don't have a population 
# size ("NA"), and are unable to be categorized as either "stable" or "threatened".
```
...
```{r}
# Proposed Question:
#   Within the species with short life spans, which habitat 
#   has the highest mean_age_at_sexual_maturity?
#   Assume that the only variables of import are 
#   life_expectancy, habitat, and mean_age_at_sexual_maturity,
#   and include n for each habitat.

ecosphere %>% 
  select(life_expectancy, habitat, mean_age_at_sexual_maturity) %>% 
  filter(life_expectancy == "Short") %>% 
  group_by(habitat) %>% 
  summarize(
    max_mean_age_at_sexual_maturity = max(mean_age_at_sexual_maturity, na.rm=T),
    n = n(),) %>% 
  arrange(-max_mean_age_at_sexual_maturity)
```
