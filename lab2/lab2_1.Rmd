...
```{r}
speed_average <- mean(c(venom_GT,chiron,veyron))
speed_average
```
...
I would interpret this result to mean that NAs will interfere with the mean() function. This means that these values should be removed before calculations.

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm=T) #na.rm will remove NA
```
It's useful because we can use it to remove the NAs so that we can properly perform calculations.
