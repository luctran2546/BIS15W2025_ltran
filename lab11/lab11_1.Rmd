...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass = mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order,mean_mass), y = mean_mass))+ #reorders order by mean_mass
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  geom_bar()+
  labs(title="Elephants Per Sex",x= "Sex",y = "Count")+
...
```{r}
life_history %>% 
  ggplot(aes(gestation, log10(mass), size=mass))+
  geom_point()
```
