...
```{r}
p + geom_point(size=1.25) #changes size of the points themselves
```
...
```{r}
p+geom_point(aes(color=thermoregulation, shape=thermoregulation), size = 1.75)
```
...
penguins %>% 
  ...
  geom_bar(position = "dodge")+
  labs(title = "Number of penguins by island, further grouped by sex")
```
