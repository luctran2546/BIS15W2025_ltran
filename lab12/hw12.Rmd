...
**How does the median life expectancy change over time? Do it for the entire planet at once. (I didn't really have much to work with. There's only six variables.)**

*It seems to increase as a whole over time.*
```{r}
gapminder %>% 
  group_by(year) %>% 
  summarize(median_life_expectancy = median(lifeExp)) %>% 
  arrange(year)

gapminder %>% 
  group_by(year) %>% 
  summarize(median_life_expectancy = median(lifeExp)) %>%
  ggplot(aes(x = year, y = median_life_expectancy))+
  geom_point()+
  geom_line()+
  labs(
