...
homerange %>% 
  ...
  geom_boxplot()+
  labs(title = "log10.mass vs Class")
```
...
homerange %>% 
  ...
  geom_boxplot()+
  labs(title = "log10.mass vs Class Grouped by Taxon")#+
...
deserts %>% 
  ...
  labs(title = "Number of Individuals by Year, by Species")+
  theme(plot.title = element_text(hjust=0.5))
```
...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_histogram(color="black", fill="wheat2")
```
...
homerange %>% 
  ggplot(aes(log10.mass))+
  geom_density(color="salmon", fill="mistyrose", alpha=0.4)+
...
homerange %>% 
  ...
  geom_histogram(fill="wheat2", color = "plum")+
  labs(title = "Histogram of log10.hra")
```
...
homerange %>% 
  ggplot(aes(log10.hra))+
  geom_density(fill="wheat2", color = "plum", alpha=0.4)+
  labs(title = "Density Plot of log10.hra")
```
...
```{r}
quartiles_1 <- quantcut(homerange$log10.hra)
table(quartiles_1)
homerange <- homerange %>% 
...
homerange %>% 
  ...
  geom_bar(pos='dodge', color = "black", alpha = 0.6)+
  labs(title = "Number of Species Group by Homerange Size and Class")+
  theme(plot.title = element_text(hjust=0.5))
```
...
homerange %>% 
  ...
  geom_boxplot()+
  labs(title = "Small Homerange Species Range Grouped by Class")+
