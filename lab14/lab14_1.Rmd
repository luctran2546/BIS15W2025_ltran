...
ui <- fluidPage( #ui = "user interface"
  
)

server <- function(input, output, session) { #server actually does stuff--stiches ui and server together
...
ui <- fluidPage(
      selectInput("x", #we are naming the select input "x"
                  "Select X Variable", #what the user will see in the app
                  choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), #possible choices
                              selected = "bill_length_mm") #default choice when app launches
)
...
ui <- fluidPage(
    ...
    selectInput("y", 
                ...
                choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), 
                selected = "body_mass_g"),
    plotOutput(outputId = "plot", width = "500px", height = "400px") #define the output
)
...
ui <- fluidPage(
  radioButtons("fill",
              ...
              c("trophic.guild", "thermoregulation"),
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ggplot(data=homerange,aes_string(x = "locomotion", fill = input$fill))+
             geom_bar(pos = 'dodge', color = "black")
  })
