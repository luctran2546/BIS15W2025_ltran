...
server <- function(input, output, session) {
  session$onSessionEnded(stopApp) #auto stopp of app when closed
  output$plot <- renderPlot({
    penguins %>% 
      ggplot(aes_string(x = input$x, y = input$y, color = "species"))+
      geom_point(na.rm=T, size = input$pointsize)
  })
}
