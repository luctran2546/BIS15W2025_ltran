...
library(tidyverse)
library(skimr) #provides summaries. see later in the lab.
library(janitor)
...
```{r}
penguins %>% 
 # group_by(island) %>% #unnecessary code
  ggplot(aes(island)) + 
  geom_bar()
```
...
homerange %>% 
  ...
  ggplot(aes(x= trophic.guild, y = log10.mass, fill=trophic.guild)) + #"fill=" adds fun colors!
  geom_boxplot()
```
...
homerange %>% 
  ...
  group_by(family) %>% 
  ggplot(aes(x = log10.mass, y = family, fill = family))+
  geom_boxplot()
```
