...
```{r}
life_history %>% 
  mutate(across(everything(), ~ifelse(. %in% c("not measured", "-999", "-999.0"), NA, .))) %>% 
  summarize(across(everything(), ~ sum(is.na(.))))

##Alternative method:

life_history_no_nas <- read_csv("data/mammal_lifehistories_v3.csv", na = c("-999", "-999.0", "-999.00", "not measured", "NA", " ", ".")) %>% 
  clean_names()
```
