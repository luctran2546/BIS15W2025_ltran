...
Each variable does not have its own income, and multiple observations are shoved into the same row.
...
```{r}
plant_data <- read.csv("data/plant_data.csv") %>% clean_names()
plant_data
```

2. Why are these data not tidy?  
The day in which it occured is ALSO data, and represents more data than first seems.
