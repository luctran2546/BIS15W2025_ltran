...
In short, **plot= data + geom\_ + aesthetics**.
...
```{r}
ggplot(data = iris,
       aes(x=Species, y=Petal.Length)) + #uses a plus and NOT a pipe
  geom_boxplot()
```
...
It has many NAs. It looks tidy (NA problem aside.)
```{r}
glimpse(homerange)
```
...
ggplot(homerange,
       aes(trophic_guild))+
  geom_bar() #used when making counts of a categorical variable
```
