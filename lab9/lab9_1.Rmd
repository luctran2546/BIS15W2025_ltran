...
tidy_expenditures <- expenditures %>% 
  pivot_longer(
    -expenditure, #do not move the expenditure value
    names_to = "year", #put years into year column
    values_to = "spending" #put the values into spending. Note that the prof uses "bn_dollars" to be clear about the value
  )
```
...
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = -1) %>% 
  mutate(patient=str_remove(patient, "_")) #stringr
```
